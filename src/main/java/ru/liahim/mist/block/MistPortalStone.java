...
public class MistPortalStone extends MistBlock {
	...
	public MistPortalStone(boolean isWork) {
		...
		this.isWork = isWork;
		if (isWork) this.setLightLevel(0.8F);
	...
	public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, ITooltipFlag advanced) {
		if (stack.getItemDamage() == 2) tooltip.add(I18n.format("tile.mist.portal_old_down.tooltip"));
		if (stack.getItemDamage() == 3) tooltip.add(I18n.format("tile.mist.portal_old_up.tooltip"));
	}
	...
	public AxisAlignedBB getCollisionBoundingBox(IBlockState blockState, IBlockAccess worldIn, BlockPos pos) {
		if (isWork) {
			if (blockState.getValue(ISUP)) return UP_AABB;
	...
	public void neighborChanged(IBlockState state, World world, BlockPos pos, Block blockIn, BlockPos fromPos) {
		if (!world.isRemote && (blockIn == MistBlocks.PORTAL || blockIn == Blocks.AIR)) {
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		if (!world.isRemote && !Dimension.loadedDimBlackList.contains(world.provider.getDimension())) {
			if (!this.isWork) {
				...
				if (portalBlock == MistBlocks.PORTAL) return;
				world.setBlockState(pos, MistBlocks.PORTAL_BASE.getDefaultState().withProperty(ISNEW, state.getValue(ISNEW)).withProperty(ISUP, isUp));
			}
		}
	}
	...
	private void createPortal(World world, BlockPos pos) {
		...
		world.setBlockState(pos.up(), MistBlocks.PORTAL_WORK.getDefaultState().withProperty(ISUP, true).withProperty(ISNEW, world.getBlockState(pos.up()).getValue(ISNEW)));
		world.setBlockState(pos.down(), MistBlocks.PORTAL_WORK.getDefaultState().withProperty(ISUP, false).withProperty(ISNEW, world.getBlockState(pos.down()).getValue(ISNEW)));
	}

	@Override
	public void getSubBlocks(CreativeTabs tab, NonNullList<ItemStack> list) {
		if (!this.isWork) {
			list.add(new ItemStack(this, 1, this.getMetaFromState(this.getDefaultState().withProperty(ISNEW, true).withProperty(ISUP, false))));
			list.add(new ItemStack(this, 1, this.getMetaFromState(this.getDefaultState().withProperty(ISNEW, true).withProperty(ISUP, true))));
			list.add(new ItemStack(this, 1, this.getMetaFromState(this.getDefaultState().withProperty(ISNEW, false).withProperty(ISUP, false))));
			list.add(new ItemStack(this, 1, this.getMetaFromState(this.getDefaultState().withProperty(ISNEW, false).withProperty(ISUP, true))));
		}
	}
